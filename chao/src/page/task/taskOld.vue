<template>
  <div class='task' >
    ...
    <el-dialog title='取消发布' width='30%' :visible.sync="hiddedialog" >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type='primary' @click="hiddedialog = false" size='small'>确认</el-button>
    ...
    <el-dialog title='暂停任务' width='30%' :visible.sync="stopdialog" >
      ...
      <el-input  type='password' v-model="stopeinput" placeholder="请输入密码"></el-input>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title='删除任务' width='30%' :visible.sync="deldialog" >
      <p class='hidde-title'>您将删除该任务发布，请输入密码确认。</p>
      <el-input  type='password' v-model="input" placeholder="请输入密码"></el-input>
      <span slot="footer" class="dialog-footer">
...
<script>
    import {taskList,taskDel,taskStope,taskCuo,maiList,down,formatDate} from '../../api/api'
    export default {
        name: "goods",
        data(){
          return{
        ...
        methods:{
          ...
          excel_down:function(){
            ...
            down(this.form.state,this.form.states,result,results,resultss,date1,date2,this.form.account,this.form.storeName,resultsss,resultssss,2,1,100000).then(response=>{
                this.down=true;
                const data=response;
                var url=data.path;
          ...
          search(){
            ...
            // console.log(date1);
            // console.log(date2);
            taskList(this.form.state,this.form.states,this.form.tai,result,results,resultss,date1,date2,this.form.account,this.form.storeName,resultsss,resultssss,2,this.currentPage,10).then(response => {
              const data = response;
               if(data.msg==1){
                this.tableData=data.page.result
                this.totalPages=data.page.totalRow
          ...
          CurrentChange(e){
            ...
            taskList(this.form.state,this.form.states,this.form.tai,result,results,resultss,date1,date2,this.form.account,this.form.storeName,resultsss,resultssss,2,e,10).then(response => {
              const data = response;
               if(data.msg==1){
                this.tableData=data.page.result
                this.totalPages=data.page.totalRow
          ...
          stopTask(){
            ...
            if(this.stopeinput==''){
              ...
              taskStope(this.taskIds,this.stopeinput).then(response=>{
