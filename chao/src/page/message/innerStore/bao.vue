          ...
          <el-table  ref="multipleTable" :data="tableData" tooltip-effect="dark"  style="width: 100%"   @selection-change='selection' header-row-class-name="table-header">
            ...
            <el-table-column prop="type" label="类型"  show-overflow-tooltip header-align="center">
              <template slot-scope="scope">
                <p v-if='scope.row.status==15'>退款</p>
                <p v-if='scope.row.status==17'>充值</p>
              </template>
            </el-table-column>
            <el-table-column label="操作"  show-overflow-tooltip header-align="center">
              <template slot-scope="scope">
                <el-button type="success" plain  size="mini" @click="missdialog = true,Ids=scope.row.id,type=1" v-if='scope.row.status==17'>充值</el-button>
                <el-button type="danger" plain  size="mini" @click="missdialog = true,Ids=scope.row.id,type=2" v-if='scope.row.status==15'>退款</el-button>
              </template>
            </el-table-column>
          </el-table>
...
<script>
    import {businessMuchList,businessZhuans,businessBaoChong,businessBaoTui} from '../../../api/message'
    ...
    export default {
        ...
        mounted(){
          ...
          businessMuchList('','','',1,10,4).then(response=>{
            var data=response
            if(data.msg==1){
        ...
        methods:{
          ...
          CurrentChange(e){
            ...
            businessMuchList(this.form.num,date1,date2,e,10).then(response=>{
              var data=response
              if(data.msg==1){
                this.tableData=data.page.result
                this.totalPages=data.page.totalRow
          ...
          search(){
            ...
            businessMuchList(this.form.num,date1,date2,this.currentPage,10).then(response=>{
              var data=response
              if(data.msg==1){
                this.tableData=data.page.result
                this.totalPages=data.page.totalRow
          ...
          confirm(){
            ...
            if(this.type==1){
              if(this.password==''){
                ...
                businessBaoChong(this.Ids,this.password).then(response=>{
                  var data=response
              ...
              if(this.password==''){
                ...
                businessBaoTui(this.Ids,this.password).then(response=>{
                  var data=response
                  this.missdialog=false
                  if(data.msg==1){
                    this.$message.success('操作成功')
