    ...
    <el-row class="contain row-bg" type="flex"  justify="center">
      <el-col>
        <div class="store-inner">
          <p class='break'><router-link to='/notic'>公告管理/发布公告</router-link></p>
...
<script>
   ...
   import {sendList} from '../../api/message'
  export default {
    data(){
      return {
        tag_list:[{'name':'商家','choose':false,id:3},{'name':'刷手','choose':false,id:2}],
        input:'',
        active:'active',
        dat: {
          content: ''
    ...
    methods:{
      ...
      returnContent () {
        ...
        sendList(this.input,this.$refs.ue.getUEContent(),type).then(response=>{
          var data=response
          if(data.msg==1){
            this.$message.success('发送成功')
            setTimeout(function(){
              that.$router.push('/messageNotic')
            },500)
          }
        }).catch(error=>{
