...
<script>
	...
	export default{
		...
		mounted(){
			user_img(5).then(response=>{
				var data=response
				if(data.info.message_img!=1){
					this.upimg=data.info.message_img
					this.up=true
				}
				this.example_img=data.example_img.message_img
			}).catch(error=>{
		...
		methods:{
			...
			next(){
				var that=this;
				if(this.upimg==''){
					this.$toast('请上传号码信息验证截图')
				}else{
					this.$indicator.open('上传中...');
					img_save(this.upimgs,'',5).then(response=>{
						this.$indicator.close();
						var data=response
						if(data.info==1){
							 this.$toast('资料完成，等待审核')
							 setTimeout(function(){
