...
<script>
	...
	export default{
		...
		mounted(){
			user_img(4).then(response=>{
				var data=response
				if(data.info.card_value!=1){
					this.upimg=data.info.card_value
					this.up=true
				}
				this.example_img=data.example_img.card_value
			}).catch(error=>{
		...
		methods:{
			...
			next(){
				if(this.upimg==''){
					...
					img_save(this.upimgs,'',4).then(response=>{
						this.$indicator.close();
						var data=response
						if(data.info==1){
							 this.$router.push('/proveFive')
						}
					}).catch(error=>{
