	...
	<div class='three'>
			...
			<el-table  ref="multipleTable" :data="tableData" tooltip-effect="dark"  style="width: 100%" border header-row-class-name="table-header" class='tables' >
	            <el-table-column prop="type" label="商品单价"  header-align="center">
	             <template slot-scope="scope">
	                <el-input type='number' size='small' v-model='scope.row.pro_money' @blur='dan(scope.$index,scope.row.pro_money)' ></el-input>
	              </template>
	            </el-table-column>
		...
		<div>
			...
			<el-table  ref="multipleTable" :data="tableDatas" tooltip-effect="dark"  style="width: 100%" border header-row-class-name="table-header" class='tables' >
              ...
              <!--<el-table-column label="选择日期" header-align="center"   >-->
                <!--<template slot-scope="scope">-->
                  <!--<el-date-picker-->
...
<script>
	...
	import {saveTaskAndPrice,getSingleMoneys,formatDate} from '../../../api/task';
	export default{
		data(){
			return{
        // pickerOptions1: {
		...
		methods:{
			...
			dan(indexs,money){
			  var that=this
			  getSingleMoneys(money).then(response=>{
			    const data=response;
			    console.log(data);
			    if(data.msg==3){
			      that.$message.warning('系统错误，请联系超管！')
          }else if(data.msg==4){
            that.$message.warning('商品单价不能为负数')
          }else{
            that.tableData[indexs].task_yong=data.commission.merchantCommission
            this.suan(indexs)
          }
        }).catch()
				// this.suan(indexs)
			...
			suan(index){
				...
				var yong_all=0
				var all_kuais=0
				// console.log(this.tableData[0].task_yong);
        // console.log(this.tableData[0].task_num);
        ...
        this.tableData[index].all=Number(this.tableData[index].task_yong)+Number(task_moneys)+Number(task_kuais)
			},
			// 删除算总价
			alls(){
				var money_all=0
			...
			next(){
			  // console.log(1);
        // console.log(formatDate(this.tableDatas[0].dateTime));
				var tabs=[];
				var allNumbers=0;
				for(var i=0;i<this.tableData.length;i++){
				...
				if(this.radios==2){
					...
					// else if(this.tableDatas[0].dateTime==''){
          //   this.$message.warning('请设置选择时间');
          // }
          else{
						this.tiaoTwo=true
					}
				...
				if(this.tiaoOne&&this.tiaoTwo){
					...
					// var date1=formatDate(this.tableDatas[0].dateTime);
