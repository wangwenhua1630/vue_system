      ...
      <el-col>
        <div class="first-box">
          ...
          <el-form ref="form" :model="form" class="queryBox" label-width="76px"  size="small" :inline="true">
            <el-form-item>
              <el-select v-model="form.tai" placeholder="所属平台"  @change="handType">
          ...
          <el-table  ref="multipleTable" :data="tableData" :row-class-name="tableRowClassName" tooltip-effect="dark"  style="width: 100%"  header-row-class-name="table-header" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="所属平台" header-align="center"  show-overflow-tooltip width="100">
              <template slot-scope="scope" v-if="scope.row.adminStoreType">
            ...
            <el-table-column  label="买号/商品信息"  show-overflow-tooltip header-align="center" width="230">
              <template slot-scope="scope">
                ...
                <p class='task-look' @click="maiLook(scope.row.handId,scope.row.handDetailId)">查看买号信息</p>
            ...
            <el-table-column   label="操作"  header-align="center" width='120' fixed="right">
              <template slot-scope="scope">
                ...
                <el-button type="danger"  plain round  size="mini" @click="cancelBtn(scope.row.id)" v-if='scope.row.handTaskStatus==1||scope.row.handTaskStatus==2||scope.row.handTaskStatus==7' >取消发布</el-button>
...
<script>
    import {findAllTaskByUserId,cancelTask,customerServiceIn,findErrorMsgByBandTaskId,taskCuo,taskBiao,maiList,down,formatDate,cancelFabu} from '../../api/task';
    export default {
        data(){
          return{
            tai:'',
        ...
        methods:{
          ...
          cancelFabu(){
            ...
            if(this.inputs==''){
              ...
              cancelFabu(this.typeFa,this.sendIds,this.inputs,this.form.states).then(
                response=>{
                  const data=response;
          ...
          search(){
            ...
            findAllTaskByUserId(1,10,this.form.state,this.form.tai,this.form.states,result,results,resultss,result4,result5,result6,date1,date2).then(response => {
              const data = response;
               if(data.msg==1){
                  this.tableData=data.page.result
                  for(var i=0;i<data.page.result.length;i++){
          ...
          CurrentChange(e){
            ...
            if(this.form.date==null||this.form.date==''){
             ...
             findAllTaskByUserId(e,10,this.form.state,this.form.tai,this.form.states,result,results,resultss,result4,result5,result6,date1,date2).then(response => {
              const data = response;
               if(data.msg==1){
                  this.tableData=data.page.result
                  for(var i=0;i<data.page.result.length;i++){
