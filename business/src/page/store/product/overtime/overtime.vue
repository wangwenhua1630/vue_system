...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      zhuan(1,10000,7).then(response => {
        const data = response;
         if(data.msg==1){
            this.tableDatasss=data.page.result
         }
      }).catch(error => {
        reject(error);
      });
    ...
    methods:{
      ...
      search(){
        ...
        zhuan(1,10,7,this.form.num,date1,date2).then(response => {
          const data = response;
           if(data.msg==1){
              this.tableData=data.page.result
              this.totalPages=data.page.totalRow
      ...
      confirm(){
        ...
        if(this.passwords==''){
          ...
          zhuanStatus(7,this.zhuan_type,idList,this.passwords,'原因').then(response => {
            const data = response;
             if(data.msg==1){
