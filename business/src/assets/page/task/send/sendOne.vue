<template>
	<div >
      ...
      <div class='typeTan' v-if='tanBox'>
        <sendType v-on:child="changeType"></sendType>
      </div>
	</div>
</template>
<script>
...
import {prodeuceList} from '../../../api/vip';
     ...
     //隐藏原图
      ...
      next(){
        if(this.chooseIndex<0){
          ...
          addTask(this.tableData[this.chooseIndex].storeId,this.tableData[this.chooseIndex].id,this.sendTypeIndex).then(response => {
            const data = response;
            if(data.msg==1){
      ...
      search(){
        ...
        prodeuceList(goods,this.form.state,goodsName,this.currentPage,10).then(response => {
          const data = response;
          this.tableData=data.page.result
          this.totalPages=data.page.totalRow
          this.storeList=data.stores
