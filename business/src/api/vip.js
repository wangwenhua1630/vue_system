...
//商品店铺详情
export function goodsUpdata(id){
  const  data=qs.stringify({
    id
  });
  return fetch({
    url:'/sd-web/goods/findGoodById',
    method:'post',
    data
  })
}
...
export function storeList(){
    return fetch({
        url: '/sd-web/store/findStoreByType'
    });  
}
// 调整单量
export function storeListDan(){
  return fetch({
...
// 店铺切换
export function storeListQie(storeTypeId){
  const data=qs.stringify({
    storeTypeId
  })
  return fetch({
    url: '/sd-web/store/findStoreByTypeId?storeTypeId='+storeTypeId,
  });
}
// 店铺列表1
...
export function prodeuceListTai(storeTypeId,currentPage,size){
  const data = qs.stringify({
    storeTypeId,
    currentPage,
    size
  ...
  return fetch({
    url: '/sd-web/task/get/storesAndGoods',
    method: 'post',
    data
  });
}
// 添加商品
export function addGoods(storeTypeId,id,storeId,goodId,url,goodName,goodNickname,img,mobileImg,command){
    const data = qs.stringify({
        storeTypeId,
        id,
        storeId,
...
//判断淘口令是否补充完整
export function taoCode() {
  return fetch({
    url:'/sd-web/goods/goodsCommand',
  })
}
